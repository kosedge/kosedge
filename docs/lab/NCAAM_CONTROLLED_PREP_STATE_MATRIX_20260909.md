...
| --- | --- | --- |
| Phase 2.6F foundation (PR A #500) | **MERGED** 2026-09-07 | merge `729c07ce…` |
| Phase 2.6F governance (PR B #501) | **MERGED** 2026-09-07 | merge `beae0013…` |
| B2-PACE-v1 challenger (PR #490) | **MERGED** 2026-09-07 | merge `9e755056…`; materialize remains incumbent-only |
| NHL Edge Board honesty (PR #504) | **MERGED** 2026-09-08 | merge `2832bc5f…` (Lane B context only) |
| Sealed holdout package | `ncaam_holdout_2024_25_v1_1` sealed; `features_labels_joined_for_evaluation=false`; `performance_metrics_calculated=false` | `data/ops/lab/ncaam/holdout_2024_25/seal/seal_receipt.json` |
...
| B2-PACE-v1 | Frozen unit-correction challenger; Train-A diagnostics + Phase 2.7A immutable HCA; **not** production default | `fair_b2_pace_v1.py`, registry |
| Neutral-site handling (pre this prep) | SoT packs carry `neutral_site`; Lab fair engines ignore it; venue contract exists for holdout foundation only | Phase 2.5 hardening + `venue_contract.py` |
| Train-A | 2022-11-07→2023-03-12 development window; parquet present | protocol + `ncaam-fair-lab-train_a-latest.parquet` |
