# NBA grade schema (desk publication)

**Phase:** Infrastructure. Not a ratings pass.  
**Stamp frozen:** `v0.1` · Ch2–Ch7  
**Store:** append-only `data/nba_grades_2026.jsonl`
...
One JSON object per line in `data/nba_grades_2026.jsonl`.
...
| `market`       | string                                      | `"spread"` \| `"total"` \| Odds-backed prop (`pts` \| `reb` \| `ast` \| `threes` \| `pra`) |
...
- **Props (displayed / Odds-backed only):** `pts`, `reb`, `ast`, `threes`, `pra` — `proj` from Ch5; `best_kick` from Ch6 Best; `kei` null; `tag` = `n/a`.
...
- This schema doc
- `data/nba_grades_2026.jsonl` (append-only)
- `scripts/nba/grade_harness.py` — seed + summarize + later tip close/final fill
...
- New means / minute-grid rewrite
- Preview copy / Chapter 8 chrome
- CFB/NFL grade files (except a shared helper if one already exists — none today)
...
Chapter 8 chrome (previews / camp) waits on media day — not this PR.
