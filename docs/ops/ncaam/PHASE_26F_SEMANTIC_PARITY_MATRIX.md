...
| Scripts (`build_*`, `hydrate_*`, `rebuild_*`, `verify_*`) | PR A′′                     | reproducible (git)                                   | FULL+ (path B + raw hard-stop) |
...
| `ncaam_official_schedule_2024_25.json` bulk pack          | not git                    | **path-B rebuildable** from governed ESPN raw        | recovery locked                |
...
- **path-B rebuildable**: sealed packages + canonical schedule pack rebuilt deterministically from governed ESPN raw (R2 hydrate) + in-repo KenPom/odds; fail-closed if inputs missing; no manual placement.
...
| Clean-checkout seal recovery    | N/A                  | path B                | inherits A′′        |
