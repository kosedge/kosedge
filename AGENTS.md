...
2. **GitHub → Railway**: Connect the repo in Railway; root directory `/services/model-service`, config `/services/model-service/railway.toml`. Watch path limits rebuilds to model-service changes.
3. **Vercel → Railway**: Set `MODEL_SERVICE_URL` on Vercel to the Railway public HTTPS URL (e.g. `https://….up.railway.app`). Set matching `INTERNAL_API_SECRET` on both.
