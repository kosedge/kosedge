...
1. **GitHub → Vercel**: Vercel GitHub integration auto-deploys on push/PR (project `kosedge`, root `apps/web`). **Production branch:** `deploy-vercel` (full Pro hubs). Never point Vercel production at `restore-working-ui` — that branch is a thinner UI shell.
2. **GitHub → Railway**: Connected as service `kosedge` in project `joyful-clarity` (`1873f728-9c20-4350-9dbe-802153ecc835`). Root `/services/model-service`, config `/services/model-service/railway.toml`. Deploy trigger: `restore-working-ui`. Public URL: `the Railway model-service public URL`. Postgres service provides `DATABASE_URL`.
3. **Vercel → Railway**: `MODEL_SERVICE_URL=the Railway model-service public URL` with matching `INTERNAL_API_SECRET` on both.
