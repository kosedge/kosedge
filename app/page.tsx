...
export default function Home() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-950 to-black text-gray-100 font-inter relative overflow-hidden">
      {/* Subtle animated bg with gold/green hints */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="w-full h-full bg-[radial-gradient(circle_at_20%_30%,rgba(34,197,94,0.12)_0%,transparent_60%),radial-gradient(circle_at_80%_70%,rgba(212,175,55,0.10)_0%,transparent_60%)] animate-pulse-slow" />
      </div>

      {/* Header – more gold in logo */}
      <header className="relative z-20 border-b border-gray-800 bg-black/60 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-5">
            <button className="px-5 py-2.5 text-sm font-medium rounded-lg border border-kos-gold/60 text-kos-gold hover:bg-kos-gold/10 transition">
              Read Insights
            </button>
            <button className="px-6 py-3 bg-gradient-to-r from-kos-green to-kos-dark-green rounded-lg font-bold text-white hover:brightness-110 transition shadow-lg shadow-kos-green/30">
              Get Edge Alerts
            </button>
          </div>
        </div>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pt-20 pb-24">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-6 mb-20">
          <button className="px-10 py-5 bg-kos-gold text-black font-bebas text-2xl rounded-xl shadow-2xl shadow-kos-gold/50 hover:scale-105 transition hover:brightness-110">
            Today's Edge Board
          </button>
          <button className="px-10 py-5 bg-gray-900/70 border border-kos-gold/50 rounded-xl font-semibold text-lg text-kos-gold hover:bg-gray-800 transition">
            How the Model Works
          </button>
        </div>

        {/* Grid – gold titles, borders */}
        <div className="grid md:grid-cols-12 gap-8">
          ...
          {/* Edge Board – gold header, accents */}
