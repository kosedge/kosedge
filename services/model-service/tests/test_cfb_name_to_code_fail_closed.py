...
def test_w2_mint_cannot_silently_keep_w0_stamps() -> None:
    assert kei_version_for_weeks((0, 1, 2)) == "cfb-kei-v1.0-2026w2"
    assert kei_version_for_weeks((0,)) == "cfb-kei-v1.0-2026w0"
    as_of = resolve_kei_as_of(weeks=(0, 1, 2), explicit=None, mint_date="2026-09-10")
    assert as_of == "2026-09-10"
    assert as_of != W0_CLOSE_AS_OF
    stale = resolve_kei_as_of(
        weeks=(0, 1, 2), explicit=W0_CLOSE_AS_OF, mint_date="2026-09-10"
    )
    assert stale == "2026-09-10"
