...
def test_known_conflicts_match_desk() -> None:
    ...
    assert rbs[("KC", "RB")]["player_name"] == "Kenneth Walker III"
    assert rbs[("SEA", "RB")]["player_name"] == "Zach Charbonnet"
    sea_rbs = sorted(
        [r for r in rows if r["team"] == "SEA" and r["position"] == "RB"],
        key=lambda r: int(r["depth_order"]),
    )
    assert sea_rbs[0]["player_name"] == "Zach Charbonnet"
    walker = [r for r in rows if r["player_name"] == "Kenneth Walker III"]
    assert len(walker) == 1 and walker[0]["team"] == "KC"
    assert int(walker[0]["depth_order"]) == 1
    charb = [r for r in rows if r["player_name"] == "Zach Charbonnet"]
    assert len(charb) == 1 and charb[0]["team"] == "SEA"
    assert int(charb[0]["depth_order"]) == 1
    sea_walker = [
        r
        for r in rows
        if r["team"] == "SEA" and r["player_name"] == "Kenneth Walker III"
    ]
    assert sea_walker == []
