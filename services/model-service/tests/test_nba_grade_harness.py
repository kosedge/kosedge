"""NBA grading harness — schema + example seed (no pack rewrite)."""
...
SCRIPT = ROOT / "scripts" / "nba" / "grade_harness.py"
...
def test_schema_exists() -> None:
    ...
    assert "nba_grades_2026.jsonl" in text
...
def test_store_schema_example_first_fill() -> None:
    assert STORE.exists(), "run: python3 scripts/nba/grade_harness.py seed"
    rows = [
        json.loads(line)
    ...
    assert "pts" in markets and "pra" in markets
