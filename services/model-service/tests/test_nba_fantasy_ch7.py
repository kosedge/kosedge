"""NBA Chapter 7 — fantasy from PlayerProjection gates."""
...
from src.services.nba_season_engine.nba_props import build_dark_props_board
...
def test_ch7_scoring_map_registered() -> None:
    ...
    assert FANTASY_VERSION == "nba-fantasy-ch7-v1"
...
def test_team_pts_drift_inside_residual_cap() -> None:
    board = build_fantasy_board(limit=10)
    assert board["max_team_pts_drift"] <= P.TEAM_REBASE_RESIDUAL_CAP
    assert board["max_team_pts_drift"] < 0.01  # live pack ~0.0002
