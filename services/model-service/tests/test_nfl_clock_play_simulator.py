...
def test_rz_q4_trail3_precedence_respects_regulation_max_clock() -> None:
    config = ClockPlayConfig(
        red_zone_fourth_decision_enabled=True,
        red_zone_q4_trail3_fg_precedence_full_quarter_enabled=True,
        q4_trail3_fg_range_precedence_max_clock_seconds=390.0,
        q4_trail3_fg_range_action_probabilities={"field_goal": 1.0, "go": 0.0},
        red_zone_fourth_decision_priors={
            "default": {
                "bucket": "unit_test",
    ...
    assert simulator._fourth_down_decision("home") == "go"
    simulator.state.clock_seconds = 360.0
    assert simulator._fourth_down_decision("home") == "field_goal"
...
def test_rz_q4_trail3_regulation_clock_blocks_early_fg_range_field_goal() -> None:
    config = ClockPlayConfig(
        ...
        q4_trail3_fg_range_precedence_soft_max_clock_seconds=327.0,
        q4_trail3_fg_range_action_probabilities={"field_goal": 1.0, "go": 0.0},
        fourth_down_continuation_enabled=True,
    )
    simulator = ClockPlaySimulator(_inputs(), seed=1, config=config)
    ...
    assert simulator._fourth_down_decision("home") == "go"
    simulator.state.clock_seconds = 300.0
    assert simulator._fourth_down_decision("home") == "field_goal"
...
def test_endgame_trail3_mid_fg_band_caps_scrimmage_touchdown() -> None:
    config = ClockPlayConfig(
        q4_endgame_trail3_mid_fg_band_block_scrimmage_touchdown=True,
        explosive_play_probability=1.0,
        base_yards=50.0,
        yards_spread=0.0,
    )
    simulator = ClockPlaySimulator(_inputs(), seed=1, config=config, collect_events=True)
    ...
    simulator._resolve_scrimmage_play("home")
    assert simulator.state.yardline == 99
    assert simulator.state.event_counts.get("touchdown", 0) == 0
...
def test_q4_trail3_margin_preservation_skips_fg_band_inside_endgame() -> None:
    config = ClockPlayConfig(
        ...
        q4_trail3_margin_preservation_field_goal_probability=1.0,
    )
    simulator = ClockPlaySimulator(_inputs(), seed=1, config=config)
    simulator.state = ClockPlayState(
        quarter=4,
    ...
    assert not simulator._q4_trail3_margin_preservation_field_goal_eligible("home")


def test_q4_trail3_margin_preservation_substitutes_field_goal_for_touchdown() -> None:
    config = ClockPlayConfig(
        q4_trail3_margin_preservation_field_goal_enabled=True,
        q4_trail3_margin_preservation_field_goal_probability=1.0,
        field_goal_max_distance=62,
    )
    simulator = ClockPlaySimulator(_inputs(), seed=1, config=config, collect_events=True)
    simulator.state = ClockPlayState(
        quarter=4,
    ...
    simulator._score_touchdown("home", source="scrimmage_play")
    assert simulator.state.home_score == 13
    assert simulator.state.away_score == 7
    assert any(
        e.get("event_type") == "q4_trail3_margin_preservation_field_goal"
...
def test_endgame_trail3_fg_range_approach_extra_yards() -> None:
    ...
    assert simulator._endgame_trail3_fg_range_approach_extra_yards("home") == 2.5
    simulator.state.yardline = 85
