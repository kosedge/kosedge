...
def test_endgame_trail3_goal_line_block_caps_scrimmage_yards() -> None:
    config = ClockPlayConfig(
        q4_endgame_trail3_fg_range_block_goal_line_touchdown=True,
    )
    simulator = ClockPlaySimulator(_inputs(), seed=1, config=config)
    simulator.state = ClockPlayState(
        quarter=4,
    ...
    assert simulator._cap_yards_for_endgame_trail3_goal_line_block("home", 20) == 3
