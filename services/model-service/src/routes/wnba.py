...
def _resolve_active_model_version(
    session: Any, fallback: str = DEFAULT_WNBA_MODEL_VERSION
) -> str:
    try:
        row = session.execute(
            text(
...
def _fetch_upcoming_games(session: Any, target_date: date) -> List[Dict[str, Any]]:
    """Prefer core games hierarchy; fall back to wnba_games_ingest."""
    try:
        ...
        log.info("WNBA games hierarchy query unavailable: %s", str(exc)[:200])
    ...
    try:
        ...
        log.info("WNBA ingest games query unavailable: %s", str(exc)[:200])
        return []
...
def wnba_fair_lines(
    ...
    try:
        ...
        try:
            if game_date is not None or days_ahead == 0:
                rows = session.execute(
                    text(
                        ...
                        FROM wnba_market_projections mp
            ...
            log.warning("WNBA fair-lines query failed (schema/runtime): %s", str(exc)[:300])
            schema_ready = False
            rows = []
            ...
            if not home_team or not away_team:
                try:
                    proj_row = session.execute(
                        text(
                            """
                            SELECT projection FROM wnba_market_projections
        ...
        if len(lines) == 0:
            ...
            message = (
                "WNBA possession sim board is live; no projections for this date. "
                "Offseason empty slate — not inventing fair prices."
                if slate_status == "offseason_empty"
                else (
                    "WNBA fair-lines schema/runtime not ready."
                    if slate_status == "schema_not_ready"
                    else "No WNBA projections stored yet for this date. Run simulations after context assemble."
                )
            )
        else:
            slate_status = "ok"
    ...
    try:
        ...
        rows = session.execute(
            text(
                ...
                FROM wnba_player_prop_model_edges
        ...
        raise HTTPException(status_code=500, detail=f"wnba_props_board_failed: {exc}") from exc
    finally:
        session.close()
...
def wnba_ops_inventory() -> Dict[str, Any]:
    ...
    from src.services.wnba_jobs import collect_wnba_db_inventory

    session = SessionLocal()
    try:
        inv = collect_wnba_db_inventory(session)
        session.commit()
        inv["worker_build_id"] = WNBA_WORKER_BUILD_ID
        inv["phase"] = "phase3"
        return inv
    except Exception as exc:
        session.rollback()
        raise HTTPException(status_code=500, detail=f"wnba_inventory_failed: {exc}") from exc
    finally:
        session.close()


@router.post("/simulations/demo")
def wnba_demo_simulation(
    home_team: str = Query("New York Liberty"),
    away_team: str = Query("Las Vegas Aces"),
    ...
    inputs = WnbaGameInputs(**defaults)
    return simulate_wnba_game(
        inputs,
...
def wnba_run_single_game_simulation(
    game_id: str,
        ...
        try:
            row = session.execute(
                text(
                    ...
                    JOIN teams away ON away.id = g.away_team_id
                    LEFT JOIN wnba_game_context c ON c.game_id = g.id::text
                    WHERE l.code = 'wnba' AND g.id::text = :game_id
                    LIMIT 1
                    """
                ),
                {"game_id": game_id},
...
def wnba_active_model() -> Dict[str, Any]:
    session = SessionLocal()
    try:
        ensure_wnba_model_tables(session)
        session.commit()
        ...
        if not row:
            return {
                "state_key": MODEL_STATE_KEY,
                "active_model_version": DEFAULT_WNBA_MODEL_VERSION,
                "previous_model_version": None,
                "reason": "default",
                "worker_build_id": WNBA_WORKER_BUILD_ID,
            }
        m = dict(row._mapping)
        m["worker_build_id"] = WNBA_WORKER_BUILD_ID
        return m
    finally:
        session.close()
