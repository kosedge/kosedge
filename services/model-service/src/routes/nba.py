...
def nba_props_board(
    ...
    model_version: str = Query("nba-props-ch6-dark-v1"),
    ...
    if src in {"season_engine", "ch6", "dark", "player_projection", "auto"}:
        ...
        # Hard clamp: dark board never ships PLAY/WATCH.
        for r in lines:
    ...
    try:
        ...
        for r in rows:
            ...
            if not isinstance(diag, dict):
                diag = {}
            # Dark clamp — stub path also ships zero PLAY.
            diag = {
                **diag,
                "tag": "PASS",
                "tag_side": None,
        ...
        return {
            ...
            "message": (
                None
                if lines
                else "No NBA stub prop edges materialized for this date."
            ),
        }
    except Exception as exc:
...
def nba_fantasy_board(
    ...
    return {
        ...
        "TEAM_REBASE_RESIDUAL_CAP": board.get("TEAM_REBASE_RESIDUAL_CAP"),
        "TEAM_CARRY_SHRINK_unchanged": board.get("TEAM_CARRY_SHRINK_unchanged"),
        "does_not": board.get("does_not") or [],
        "phase": "ch7_fantasy",
