...
def job_mlb_sp_talent_ablation(
    ...
    configs: str = Query(
        "T0,T3",
        description="Comma-separated: T0 era/whip, T1 fip, T2 xfip, T3 stuff_proxy, B1 bullpen role",
    ),
    base_model_version: str = Query("mlb-v1-pa-sim"),
) -> Dict[str, str]:
...
@app.post("/api/jobs/mlb-lineup-timing-ablation")
def job_mlb_lineup_timing_ablation(
    start_date: str = Query("2026-05-20", description="YYYY-MM-DD densify start"),
    end_date: str = Query("2026-07-17", description="YYYY-MM-DD densify end"),
    simulations: int = Query(2000, ge=500, le=10000),
    max_games: int = Query(1200, ge=1, le=2000),
    ...
    configs: str = Query(
        "L0,L1",
        description="Comma-separated: L0 timing off, L1 sharp per-side confirm",
    ),
    base_model_version: str = Query("mlb-v1-pa-sim"),
) -> Dict[str, str]:
    """Densify lineup/nowcast timing ablation. Writes to `{base}-timing-*` versions."""
    try:
        cfg_list = [c.strip() for c in (configs or "").split(",") if c.strip()]
        async_result = celery_app.send_task(
            TASK_MLB_LINEUP_TIMING_ABLATION,
            kwargs={
                "start_date": start_date,
                "end_date": end_date,
                "simulations": int(simulations),
