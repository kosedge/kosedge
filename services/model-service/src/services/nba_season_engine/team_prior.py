"""NBA Chapter 1 — team prior shell reader.

Formula:
  team' = league_mean + TEAM_CARRY_SHRINK * (team_2025_26 - league_mean)
...
PACK_PATH = DATA_DIR / "nba_team_prior_2025_26_carry_2026_27.json"
...
def apply_team_carry_shrink(
    value: float,
    league_mean: float,
    *,
    shrink: Optional[float] = None,
) -> float:
    s = float(P.TEAM_CARRY_SHRINK if shrink is None else shrink)
...
def documentation() -> Dict[str, Any]:
    pack = load_team_prior_pack()
    return {
        "module": "src.services.nba_season_engine.team_prior",
        "engine_version": pack.get("engine_version") or P.ENGINE_VERSION,
        "TEAM_CARRY_SHRINK": P.TEAM_CARRY_SHRINK,
