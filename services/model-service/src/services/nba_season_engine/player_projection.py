"""NBA Chapter 5 — PlayerProjection reader (single scorer).

Opening-night means from Ch2 minutes × decayed box rates × team pace.
Team Σ PTS identity-scaled to Ch2 implied_ppg within TEAM_REBASE_RESIDUAL_CAP.
No props tags, no fantasy UI, no new minute grid.
"""
...
PACK_PATH = DATA_DIR / "nba_player_projection_2026.json"
...
def get_team_projections(team: str) -> List[Dict[str, Any]]:
    ...
    rows.sort(key=lambda r: (-float(r.get("MIN") or 0), str(r.get("player_name") or "")))
    return rows
...
def team_pts_identity(team: str) -> Dict[str, float]:
    ...
    return {
        ...
        "residual_cap": float(
            checks.get("residual_cap") or pack.get("TEAM_REBASE_RESIDUAL_CAP") or P.TEAM_REBASE_RESIDUAL_CAP
        ),
    }
...
def documentation() -> Dict[str, Any]:
    pack = load_player_projection_pack()
    return {
        "module": "src.services.nba_season_engine.player_projection",
