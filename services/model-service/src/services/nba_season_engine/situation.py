"""NBA Chapter 3 — situation classes (apply-on-read).
...
COEFF_PATH = DATA_DIR / "nba_situation_coeffs_v0.json"
SCHED_PATH = DATA_DIR / "nba_schedule_2025_26.json"
VENUE_PATH = DATA_DIR / "nba_venue_flags.json"
PAPER_PATH = DATA_DIR / "nba_situation_paper_sim_ch3.json"
...
def situation_delta_pts(flags: Dict[str, Any], coeffs: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
...
def apply_situation_team_line(
    ...
    *,
    ...
    return {
        ...
        "within_residual_cap": abs(adj["delta_pts"]) <= P.TEAM_REBASE_RESIDUAL_CAP + 1e-9,
    }
...
def apply_situation_player_projections(
    ...
    *,
    ...
    """Copy-through PlayerProjection PTS only when situation Δ ≠ 0 (cap-driven identity).
    ...
    # Copy-through only if the cap path moved the team line.
    if abs(delta) < 1e-12 or base_sum <= 0:
        ...
        return {
            ...
            <= P.TEAM_REBASE_RESIDUAL_CAP + 1e-6
            or abs(delta) <= P.TEAM_REBASE_RESIDUAL_CAP + 1e-9,
            "team_line": line,
            "players": players,
        }
    ...
    for r in rows:
        ...
        if "PTS" in sig:
            ...
            sig["PRA"] = round(
                (sig["PTS"] ** 2 + float(sig.get("REB") or 0) ** 2 + float(sig.get("AST") or 0) ** 2)
                ** 0.5,
                4,
            )
            sig["PR"] = round((sig["PTS"] ** 2 + float(sig.get("REB") or 0) ** 2) ** 0.5, 4)
        out["sigma"] = sig
    ...
    return {
        ...
        "within_residual_cap": abs(delta) <= P.TEAM_REBASE_RESIDUAL_CAP + 1e-9,
        "team_line": line,
        "players": players,
    }
