...
# ESPN abbreviations → kosedge packaged codes (name map preferred when present).
# Codes absent from packaged priors stay as-is and are dropped at resolve time.
...
# Safer name → code (overrides bad abbr collisions).
...
# Do NOT invent peer substitutions (e.g. Missouri≠Ole Miss) — drop instead.
...
def resolve_team_code(
    *,
    abbr: str,
    name: str,
    known_codes: Mapping[str, Any],
) -> Optional[str]:
    """Map ESPN team identity → packaged engine code (or None if FCS/unknown)."""
    name = (name or "").strip()
    abbr_u = (abbr or "").strip().upper()
    if name in ESPN_NAME_TO_CODE:
        code = ESPN_NAME_TO_CODE[name]
    else:
        # Fuzzy: strip mascot-ish trailing token failures already covered.
        code = ESPN_ABBR_TO_CODE.get(abbr_u, abbr_u)
    code = PACKAGED_CODE_ALIASES.get(code, code)
    if code in known_codes:
        return code
    # Last chance: abbr directly in known
