...
def games_from_blob(blob: Dict[str, Any], *, season: int = 2026) -> List[ScheduledGame]:
    ...
    for row in blob.get("games") or []:
        ...
        try:
            home_score_f = float(home_score) if home_score is not None else None
        except (TypeError, ValueError):
            home_score_f = None
        try:
            away_score_f = float(away_score) if away_score is not None else None
        except (TypeError, ValueError):
            away_score_f = None
        status = str(row.get("status") or "").strip().lower()
        if home_score_f is not None and away_score_f is not None and not status:
            status = "final"
        games.append(
            ScheduledGame(
                ...
                home_score=home_score_f,
                away_score=away_score_f,
                status=status,
            )
        )
