"""WNBA prop edge policy — research-only until holdout clears.

Never invents edges without a joined book line. PLAY is not stake-eligible.
Role-collapse Under refusal required (usage concentration often higher than NBA).
"""
...
from src.services.wnba_player_prop_projection import WNBA_PROP_MARKETS

PLAY_STAKE_ELIGIBLE = False
POLICY_VERSION = "wnba_props_phase3_research_v1"
...
def evaluate_wnba_prop_edge(
    *,
    ...
    if mk not in WNBA_PROP_MARKETS:
        return {
            "tag": "PASS",
    ...
    z = (mean - line_f) / std
    over_prob = (
        1.0 - _normal_cdf((line_f + 0.5 - mean) / std)
        if mk == "threes"
        else 1.0 - _normal_cdf((line_f - mean) / std)
    )
...
def ou_balance_report(rows: list) -> Dict[str, Any]:
    play = [
        r
        for r in rows
        if str((r.get("diagnostics") or {}).get("tag") or r.get("tag") or "") == "PLAY"
    ]
    over_n = sum(
        1
        for r in play
        if str((r.get("diagnostics") or {}).get("tag_side") or r.get("tag_side") or "")
        == "OVER"
    )
    under_n = sum(
        1
