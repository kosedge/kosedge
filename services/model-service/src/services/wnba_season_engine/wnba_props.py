"""WNBA Chapter 6 — props desk (dark only).
...
Odds-backed markets only (basketball_wnba). Missing Odds key → missing
(do not guess). Current Odds client keys: pts reb ast threes.
PRA / PR / RA exist on Ch5 but Odds does not return them for WNBA → not boarded.
"""
...
PROPS_VERSION = "wnba-props-ch6-dark-v1"
POLICY_VERSION = "wnba_props_ch6_dark_v1"

# Register (enterprise plan) — coded for gates/docs; dark mode never emits PLAY/LEAN.
PROP_PLAY_ABS = 4.0
PROP_PLAY_SIGMA = 0.6
PROP_PLAY_CAP_PER_SLATE = int(P.PROP_PLAY_CAP_PER_SLATE)  # 4
PROP_MINUTES_GATE = 10.0  # 40-min game; align phase3 min_minutes

# Odds client already maps these for basketball_wnba (enterprise_training_pull).
ODDS_BACKED_MARKETS = ("pts", "reb", "ast", "threes")
# Ch5 vectors present but Odds client has no WNBA key → missing (not boarded).
ODDS_MISSING_VECTORS = ("PRA", "PR", "RA")

WNBA_PROP_MARKETS = ODDS_BACKED_MARKETS
...
DARK_ONLY = True
# Absurd vs proj — WNBA-point (lower scoring than NBA 40).
ABSURD_VS_PROJ = 25.0
...
def game_level_std(market_key: str, mean: float) -> float:
    ...
    if mk == "threes":
        return _clamp(0.45 * math.sqrt(m) + 0.45, 0.55, 3.5)
    if mk == "ast":
        return _clamp(0.40 * math.sqrt(m) + 0.55, 0.7, 4.5)
    if mk == "reb":
        return _clamp(0.35 * math.sqrt(m) + 0.65, 0.8, 6.0)
    return _clamp(0.35 * math.sqrt(m) + 0.7, 1.0, 9.0)
...
def trust_prop_best(
    *,
    ...
    if off_window:
        return {"trusted": False, "best": None, "reason": "off_window"}
    if best is None:
...
def evaluate_dark_prop(
    *,
    ...
    if over_price is not None and under_price is not None:
        try:
            from src.services.wnba_prop_edge_policy import devig_two_way
...
def build_dark_props_board(
    *,
    ...
    markets: Sequence[str] = (mk_filter,) if mk_filter else WNBA_PROP_MARKETS
    if mk_filter and mk_filter not in WNBA_PROP_MARKETS:
        return {
            "present": True,
            "props_version": PROPS_VERSION,
            "count": 0,
    ...
    def _sort_key(r: Dict[str, Any]) -> Tuple[int, float, float]:
        ...
        return (
            0,
            -abs(mean - float(best)),
            -float((r.get("diagnostics") or {}).get("minutes") or 0),
        )
...
def documentation() -> Dict[str, Any]:
    return {
        "module": "src.services.wnba_season_engine.wnba_props",
        "props_version": PROPS_VERSION,
        "policy_version": POLICY_VERSION,
        "dark_only": DARK_ONLY,
        "PROP_PLAY_ABS": PROP_PLAY_ABS,
        ...
        "PROP_PLAY_CAP_PER_SLATE": PROP_PLAY_CAP_PER_SLATE,
        "PROP_MINUTES_GATE": PROP_MINUTES_GATE,
        "markets": list(WNBA_PROP_MARKETS),
        "odds_missing_vectors": list(ODDS_MISSING_VECTORS),
