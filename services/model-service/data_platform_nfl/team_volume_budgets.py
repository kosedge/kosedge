...
# Legacy alias — Phase 2 emptied named-team pass identity overlays.
...
# Phase 2: ARI / SEA / WAS added so scheme lives here, not pass soft clamps.
...
def _apply_general_volume_features(
    raw: Mapping[str, Tuple[float, float]],
    strengths: Mapping[str, Mapping[str, float]],
) -> Dict[str, Tuple[float, float]]:
    """QB rush / OL YPA multipliers from strengths payload (Phase 2)."""
    if not raw:
        return {}
    out: Dict[str, Tuple[float, float]] = {}
    for team, (pass_y, rush_y) in raw.items():
        st = strengths.get(str(team)) or {}
        pass_m = float(st.get("qb_pass_volume_mult", 1.0) or 1.0)
        rush_m = float(st.get("qb_rush_volume_mult", 1.0) or 1.0)
        ol_ypa = float(st.get("ol_ypa_mult", 1.0) or 1.0)
        tilt = float(st.get("qb_script_run_tilt", 0.0) or 0.0)
        y_pass = float(pass_y) * pass_m * ol_ypa
        y_rush = float(rush_y) * rush_m
        if tilt > 0:
            shift = y_pass * tilt * 0.35
            y_pass -= shift
            y_rush += shift
        # Returning-QB prior blend when supplied.
        prior = st.get("pass_yards_prior")
        travel = float(st.get("continuity_travel", 0.0) or 0.0)
        if prior is not None and float(prior) > 0 and travel > 0:
            w = min(0.70, 0.70 * travel)
            y_pass = w * float(prior) + (1.0 - w) * y_pass
        out[team] = (y_pass, y_rush)
    return out
...
def compute_team_season_budgets(
    ...
    adjusted = _apply_general_volume_features(raw, strengths)
