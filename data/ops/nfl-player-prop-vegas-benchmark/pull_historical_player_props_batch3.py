"""Batch 3: grow the historical player-prop sample with another disjoint
residue class (offset=1) after batches 1 (offset=0) and 2 (offset=4).

Always de-dupes against `nfl_player_prop_market_snapshots` so credits are
never spent on games already in the DB. Persists every successful pull with
`source='odds_api_historical'` and records the Odds API credit ledger.

Budget: spend up to MAX_SPEND_BUDGET credits while staying above
MIN_CREDITS_FLOOR. Cap planned games via MAX_GAMES if set.

Usage: /Users/ryankos/kosedge/.venv/bin/python3 pull_historical_player_props_batch3.py
"""
...
SAMPLE_STRIDE = 8
SAMPLE_OFFSET = 1  # disjoint from batch1 offset=0 and batch2 offset=4

MIN_CREDITS_FLOOR = 4500
MAX_SPEND_BUDGET = 2000
MAX_GAMES = 55  # ~30 credits/event-odds + date-list overhead
SLEEP_BETWEEN_CALLS = 0.15

OUTPUT_DIR = Path(__file__).parent
RUN_LOG_PATH = OUTPUT_DIR / "pull_run_log_batch3.json"
...
def record_ledger(
    ...
    *,
    ...
    signature = hashlib.sha256(
        json.dumps({"endpoint": endpoint, "params": stable_params, "batch": "3"}, sort_keys=True).encode("utf-8")
    ).hexdigest()
...
def load_sample_games(conn: psycopg.Connection) -> List[Dict[str, Any]]:
    ...
    if n_dropped:
        print(f"[dedup] dropped {n_dropped} already-pulled games from the batch-3 plan (defense-in-depth guard fired)")
    if MAX_GAMES is not None and len(deduped) > MAX_GAMES:
        print(f"[plan] capping from {len(deduped)} to MAX_GAMES={MAX_GAMES}")
...
def main() -> None:
    ...
    run_log: Dict[str, Any] = {
        ...
        "min_credits_floor": MIN_CREDITS_FLOOR,
