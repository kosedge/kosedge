"""Batch 4 HOLDOUT: residue offset=2 — never used to fit enterprise v2 tags/cal.

Purpose: validate the locked stake rule `calibrated_yards_high_z_v1` on a
fresh sample. Do NOT refit calibration intercepts or PLAY thresholds from
this batch.

De-dupes against DB. Persists with source='odds_api_historical'.

Usage: /Users/ryankos/kosedge/.venv/bin/python3 pull_historical_player_props_batch4_holdout.py
"""
...
SAMPLE_STRIDE = 8
SAMPLE_OFFSET = 2  # holdout — disjoint from 0/1/4 already pulled

MIN_CREDITS_FLOOR = 2500
MAX_SPEND_BUDGET = 2000
MAX_GAMES = 55
SLEEP_BETWEEN_CALLS = 0.15

OUTPUT_DIR = Path(__file__).parent
RUN_LOG_PATH = OUTPUT_DIR / "pull_run_log_batch4_holdout.json"
...
def record_ledger(
    ...
    *,
    ...
    signature = hashlib.sha256(
        json.dumps({"endpoint": endpoint, "params": stable_params, "batch": "4"}, sort_keys=True).encode("utf-8")
    ).hexdigest()
...
def load_sample_games(conn: psycopg.Connection) -> List[Dict[str, Any]]:
    ...
    if n_dropped:
        print(f"[dedup] dropped {n_dropped} already-pulled games from the batch-4 holdout plan")
...
def main() -> None:
    ...
    run_log: Dict[str, Any] = {
        "batch": 4,
        "purpose": "enterprise_v2_locked_rule_holdout",
