"""Batch 7: densify ALL missing 2025 W4-17 games (and any missing residue) for product board.
...
TARGET_WEEKS = [(2025, w) for w in range(4, 18)]
MIN_CREDITS_FLOOR = 1000
MAX_SPEND_BUDGET = 200000
SLEEP_BETWEEN_CALLS = 0.12

OUTPUT_DIR = Path(__file__).parent
RUN_LOG_PATH = OUTPUT_DIR / "pull_run_log_batch7_densify_2025.json"
...
def record_ledger(
    ...
    *,
    ...
    signature = hashlib.sha256(
        json.dumps({"endpoint": endpoint, "params": stable_params, "batch": "7"}, sort_keys=True).encode("utf-8")
    ).hexdigest()
    conn.execute(
        """
        ...
        """,
        {
            ...
            "status": meta["status"],
            "source_key": "the-odds-api-historical-props-batch7",
...
def live_credits_remaining(api_key: str) -> Optional[int]:
    """Prefer live Odds API headers over a possibly-stale local ledger (key rotation)."""
    payload, meta = call_odds_api("sports", {}, api_key)
    return _safe_int(meta.get("credits_remaining"))


def main() -> None:
    api_key = load_api_key()
    conn = psycopg.connect(DATABASE_URL, autocommit=True)
    credits_start = live_credits_remaining(api_key)
    if credits_start is None:
        row = conn.execute(
            "SELECT credits_remaining FROM odds_api_credit_ledger ORDER BY requested_at DESC LIMIT 1"
        ).fetchone()
        credits_start = int(row[0]) if row and row[0] is not None else None
    print(f"[budget] credits_remaining start={credits_start} (live)")
    if credits_start is not None and credits_start < MIN_CREDITS_FLOOR:
        print("[budget] below floor — abort")
        return
    # Seed ledger so subsequent tooling sees the rotated key balance.
    if credits_start is not None:
        record_ledger(
            ...
            meta={
                ...
                "credits_last": 1,
                "credits_used": None,
                "credits_remaining": credits_start,
                "error": None,
            },
            events_count=0,
        )
    ...
    run_log: Dict[str, Any] = {
        "batch": 7,
        "purpose": "densify_all_missing_2025_w4_w17",
