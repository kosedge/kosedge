"""Batch 5 CONFIRMATION: residue offset=3 — second holdout for provisional v1.2.

Locked rule under test (zero refit):
  calibrated_rec_z060_v1_2_provisional
  = rec_yds only, calibrated |z|>=0.60, edge>=0.045, |z|<1.15, gap gate

De-dupes against DB. Persists with source='odds_api_historical'.

Usage: /Users/ryankos/kosedge/.venv/bin/python3 pull_historical_player_props_batch5_confirm.py
"""
...
SAMPLE_STRIDE = 8
SAMPLE_OFFSET = 3  # confirmation — disjoint from 0/1/2/4

MIN_CREDITS_FLOOR = 800
MAX_SPEND_BUDGET = 2000
MAX_GAMES = 55
SLEEP_BETWEEN_CALLS = 0.15

OUTPUT_DIR = Path(__file__).parent
RUN_LOG_PATH = OUTPUT_DIR / "pull_run_log_batch5_confirm.json"
...
def record_ledger(
    ...
    *,
    ...
    signature = hashlib.sha256(
        json.dumps({"endpoint": endpoint, "params": stable_params, "batch": "5"}, sort_keys=True).encode("utf-8")
    ).hexdigest()
...
def load_sample_games(conn: psycopg.Connection) -> List[Dict[str, Any]]:
    ...
    if n_dropped:
        print(f"[dedup] dropped {n_dropped} already-pulled games from the batch-5 confirm plan")
...
def main() -> None:
    ...
    run_log: Dict[str, Any] = {
        "batch": 5,
        "purpose": "enterprise_v1_2_confirmation_holdout",
        "locked_rule": "calibrated_rec_z060_v1_2_provisional",
