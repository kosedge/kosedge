...
import { fetchOddsComparison, ALLOWED_BOOKS, bookDisplay } from "@/lib/odds-api";
...
export default async function OddsComparePage({
  ...
  const hasWidget = env.ODDS_WIDGET_ACCESS_KEY?.trim() && WIDGET_SPORTS.includes(sportKey);
    ...
    <div className="min-h-screen bg-[#070A0F] text-gray-100 font-inter relative overflow-hidden">
      ...
      <main className="relative z-10 w-full px-5 sm:px-6 pt-10 pb-16">
        ...
        <section className="mt-6">
          <h2 className="text-lg font-bebas text-kos-gold tracking-wide mb-3">Live odds widget</h2>
          {hasWidget ? (
        ...
        <section className="mt-8">
          <h2 className="text-lg font-bebas text-kos-gold tracking-wide mb-3">Compare odds by book</h2>
              ...
              <table className="min-w-[900px] w-full text-sm tabular-nums">
                <thead className="bg-white/5 text-gray-300 uppercase tracking-wide text-xs">
                  <tr className="text-left">
                    ...
                    {books.map((b) => (
                      <th key={b.key} colSpan={2} className="py-3 px-2 text-center border-l border-white/10">
                        {b.label}
                      </th>
                    ))}
                  </tr>
                  <tr className="text-left text-gray-500 text-[10px]">
                    ...
                    {books.map((b) => (
                      <th key={b.key} colSpan={2} className="py-1 px-2 text-center border-l border-white/10">
                        Spread | Total
                      </th>
                    ))}
                  </tr>
                    ...
                    <tr key={r.id} className="hover:bg-white/5">
                      ...
                      {books.map((b) => {
                        ...
                        return (
                          ...
                          >
                            <span className="text-kos-gold">{spread?.away ?? "—"}</span>
