...
import {
  NFL_CLUB_DESK_LABEL,
  displayClubDeskTitle,
} from "@/lib/nfl-club-desk-routes";
...
export const metadata: Metadata = {
  title: "NFL Club Desk",
  description:
    "KosEdge daily club desk — league wrap and team notes. Pass is first-class.",
};

/**
 * Day JSON has no author/byline field. Do not invent one on the page.
...
function ClubNoteCard({ card }: { card: CampDeskCard }) {
  ...
  return (
    ...
    >
      ...
      <h2 className="mt-2 text-lg font-semibold leading-snug text-kos-text sm:text-xl">
        {displayClubDeskTitle(card.title)}
      </h2>
...
export default async function NflClubDeskPage({
  searchParams,
}: {
  ...
  const current = resolveCurrentNflRegWeek();
  const truth = resolveNflTruthLabel({
    season: NFL_PRODUCT_SEASON,
    week: current.week,
    inSeason: current.week != null,
    launchPreseason: false,
  });
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-kos-gold">
              NFL Pro · {NFL_CLUB_DESK_LABEL}
            </p>
            <NflTruthStateBadge state={truth.ui_state} />
            {truth.week_label ? (
              <span
                className="text-[11px] font-semibold uppercase tracking-[0.12em] text-kos-text/55"
                data-testid="club-desk-week"
              >
                {truth.week_label}
              </span>
            ) : null}
          ...
          <p className="mt-2 max-w-2xl text-sm text-kos-text/75">
            ...
            do not invent a lean from thin noise. {truth.period_line}.
          </p>
        </div>
      ...
      <section className="mt-8 space-y-4" data-testid="camp-desk-shelf">
        ...
        {wrap ? <ClubNoteCard card={wrap} /> : null}
        {notes.map((card) => (
          <ClubNoteCard key={card.id} card={card} />
        ))}
