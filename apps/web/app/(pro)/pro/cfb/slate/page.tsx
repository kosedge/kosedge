...
export default async function CfbOfficialSlatePage({
  ...
  const board = status.desk?.week_board;
  const games = (board?.games ?? []).filter((g) => g.week === week);
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-xs text-kos-text/55">
        ...
        {status.schedule_as_of ? ` · as_of ${status.schedule_as_of}` : ""}
      </p>

      {status.error ? (
        <p className="mt-4 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs text-kos-text/65">
          Status probe failed ({status.error}). Slate rows unavailable.
        </p>
      ) : games.length === 0 ? (
