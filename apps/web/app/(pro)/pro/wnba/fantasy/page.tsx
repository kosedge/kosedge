...
import { fetchWnbaFantasyBoard } from "@/lib/wnba-fantasy-board";
...
export default async function WnbaFantasyPage({
  searchParams,
}: {
  ...
  const board = await fetchWnbaFantasyBoard({ view, limit: 120 });
      ...
      <div className="rounded-2xl border border-kos-border bg-kos-surface/30 p-6 sm:p-8">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex gap-2 text-xs">
            <Link
              href="/pro/wnba/fantasy?view=season"
            ...
            <Link
              href="/pro/wnba/fantasy?view=slate"
              className={`rounded-lg border px-3 py-1.5 ${
                view === "slate"
                  ? "border-kos-gold/50 bg-kos-gold/10 text-kos-gold"
                  : "border-white/15 text-kos-text/70"
        ...
        <p className="mt-2 text-sm text-kos-text/70">
          ...
          · {board.fantasyVersion}
          {board.seasonGames != null ? ` · ×${board.seasonGames} games` : ""}.
          Box stats are Ch5 fields; fantasy_pts applies the published map only.
        </p>
        ...
        <p className="mt-3 text-sm text-kos-text/60">
          {board.error
            ...
            : board.count > 0
              ? `${board.count} players · max team ΣPTS drift ${board.maxTeamPtsDrift ?? "—"} (cap ${board.residualCap ?? 3}) · Σ MIN = ${board.minuteGridSum ?? 200}`
