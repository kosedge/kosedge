...
function buildHref(base: Record<string, string | undefined>): string {
  ...
  return query ? `/pro/wnba/fair-lines?${query}` : "/pro/wnba/fair-lines";
}

export default async function WnbaFairLinesPage({
  searchParams,
}: {
  ...
  const board = await fetchWnbaFairLines({ gameDate, daysAhead: 5 });
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-2xl">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-kos-gold">
              WNBA Fair Lines{board.gameDate ? ` · ${board.gameDate}` : ""} · ET
            </p>
      ...
      <section className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-4 sm:p-5">
        ...
        {emptyHonest ? (
          <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-5 text-sm text-kos-text/70">
            {board.message ??
              "WNBA possession sim board is connected. No projections for this date yet — offseason empty slate is intentional, not a stub price."}
          </div>
        ) : null}

        {!board.error && !emptyHonest && lines.length === 0 ? (
          <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-5 text-sm text-kos-text/70">
            No WNBA projections for this date yet. Check back after the daily sim
            cycle.
          </div>
        ) : null}
      ...
      <section className="mt-6 rounded-2xl border border-white/10 bg-black/25 p-4 text-sm text-kos-text/70">
        <p>
          WNBA fair lines are simulation-backed reference prices from the
...
function FairLineRow({ row }: { row: WnbaFairLineRow }) {
  return (
