...
def parent_impl_sha256() -> str:
    path = Path(__file__).resolve().parent / "fair_b2_pace_v1.py"
    h = hashlib.sha256()
...
def assert_parent_immutability_pins() -> None:
    ...
    got = parent_impl_sha256()
    if got != PARENT_PACE_IMPL_SHA256:
        raise AssertionError(
            f"B2-PACE-v1 impl hash drifted: got {got} expected {PARENT_PACE_IMPL_SHA256}"
        )
