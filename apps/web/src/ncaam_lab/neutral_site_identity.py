...
Fail-closed: missing / non-boolean / ambiguous / conflicting keys never become
home or neutral. Non-boolean values are NOT coerced (bool("false") is True).
"""
...
# Frozen parent / holdout-foundation pins (content hashes on deploy-vercel @ 4bd0f87d).
PARENT_PACE_IMPL_SHA256 = (
    "4a305870fbc55900336566531eca92cd4586d8a04edd62845bcc23a03112f5bd"
)
VENUE_CONTRACT_SHA256 = (
    "9f48eb266c362c6ae5ed52ed6872293a8388ccb2611bcae05506ff0fb6f8486f"
)
...
def parse_neutral_site_flag(raw: Any) -> Tuple[Optional[bool], Optional[str]]:
    """Accept only Python bool. Everything else is unknown (fail closed).

    Strings/ints are refused: ``bool("false")`` is True and would invert identity.
    """
    if raw is None:
        return None, "missing_neutral_site"
    if raw is True:
        return True, None
    if raw is False:
        return False, None
    return None, "non_boolean_neutral_site"


def classify_venue_status(
    *,
    neutral_site_raw: Any,
    ...
    Non-boolean flags are refused before the contract (do not coerce).
    """
    parsed, parse_reason = parse_neutral_site_flag(neutral_site_raw)
    if parse_reason is not None:
        return {
            "venue_status": VENUE_STATUS_UNKNOWN,
            "neutral_site_raw": None,
            "conflict_reason": parse_reason,
            "validation_status": "unknown_fail_closed",
        }
    return normalize_venue_status(
        neutral_site_raw=parsed,
...
def load_schedule_venue_rows(
    ...
    receipt: Dict[str, Any] = {
        ...
        "accepted_raw_types": ["bool"],
        "non_boolean_policy": "unknown_fail_closed_no_coercion",
        "packs": [],
        "n_ambiguous_keys_dropped": 0,
        "n_non_boolean_flags": 0,
        "known_limitations": [
            ...
            "no secondary independent venue ledger in this lane",
        ],
    }
    ...
    for path in paths:
        ...
        for g in games:
            ...
            if not hid or not aid:
                continue
            raw_flag = g.get("neutral_site") if "neutral_site" in g else None
            parsed, parse_reason = parse_neutral_site_flag(raw_flag)
            if parse_reason == "non_boolean_neutral_site":
                n_non_bool += 1
            key = (tip.isoformat(), hid, aid)
            key_counts[key] = key_counts.get(key, 0) + 1
            norm = classify_venue_status(
                neutral_site_raw=raw_flag,
            ...
            rows.append(
                {
                    ...
                    "venue_name": g.get("venue"),
                    "season_type": g.get("season_type"),
                    "conflict_reason": norm.get("conflict_reason") or parse_reason,
                    "source_pack": path.name,
                }
            )
        receipt["packs"].append(info)

    receipt["n_non_boolean_flags"] = int(n_non_bool)
    ...
    if not clean:
        return pl.DataFrame(schema=schema), receipt
    return (
        pl.DataFrame(
            {
