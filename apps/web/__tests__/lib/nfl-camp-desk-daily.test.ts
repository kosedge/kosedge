...
describe("Sep 1 live Camp Desk day", () => {
  ...
  it("archives behind Sep 2 weekday when both are on the shelf", () => {
    const wednesday = JSON.parse(
      readFileSync(
        path.join(
          __dirname,
          "../../../../content/writers/camp-desk-2026/2026-09-02.json",
        ),
        "utf8",
      ),
    ) as CampDeskDayFile;
    ...
    const shelf = partitionCampDeskShelf(
      [...cardsFromDayFile(live), ...cardsFromDayFile(wednesday)],
      { now, inCamp: true },
    );
    expect(shelf.live[0]?.desk_date).toBe("2026-09-02");
    expect(shelf.live.every((card) => card.desk_date === "2026-09-02")).toBe(
      true,
    );
    expect(shelf.activeDeskDate).toBe("2026-09-02");
    expect(shelf.archive.some((card) => card.desk_date === "2026-09-01")).toBe(
      true,
    );
...
describe("Sep 2 live Camp Desk day", () => {
  const live = JSON.parse(
    readFileSync(
      path.join(
        __dirname,
  ...
  it("is a weekday daily package with four real-news clubs, singular preview_delta, and no X profile sources", () => {
    expect(live.desk_date).toBe("2026-09-02");
    expect(live.package).toBe("daily");
    expect(live.pinned).toBe(false);
    expect(live.source_type).toBe("kosedge-desk");
    expect(live.league_wrap.title).toBe("Camp Desk — Wednesday, Sep 2");
    expect(live.league_wrap.storylines.length).toBe(4);
    expect(live.team_notes.map((note) => note.team_id)).toEqual([
      "SF",
      "LV",
      "TB",
    ...
    expect(live.team_notes.some((note) => note.team_id === "LV")).toBe(true);
  });

  it("surfaces as the newest live package over Sep 1 daily", () => {
    const tuesday = JSON.parse(
      readFileSync(
        path.join(
          __dirname,
          "../../../../content/writers/camp-desk-2026/2026-09-01.json",
    ...
    const now = new Date("2026-09-02T22:00:00Z");
    const shelf = partitionCampDeskShelf(
      [...cardsFromDayFile(tuesday), ...cardsFromDayFile(live)],
      { now, inCamp: true },
    );
    expect(shelf.live[0]?.desk_date).toBe("2026-09-02");
    expect(shelf.live.every((card) => card.desk_date === "2026-09-02")).toBe(
...
describe("Sep 16 live Club Desk day", () => {
  const live = JSON.parse(
    readFileSync(
      path.join(
        __dirname,
  ...
  it("is a weekday daily package with nine real-news clubs, singular preview_delta, and no X profile sources", () => {
    expect(live.desk_date).toBe("2026-09-16");
    expect(live.package).toBe("daily");
    expect(live.pinned).toBe(false);
    expect(live.source_type).toBe("kosedge-desk");
    expect(live.league_wrap.title).toBe("Club Desk — Wednesday, Sep 16");
    expect(live.league_wrap.storylines.length).toBeGreaterThanOrEqual(6);
    expect(live.league_wrap.storylines.length).toBeLessThanOrEqual(8);
    expect(live.team_notes.map((note) => note.team_id)).toEqual([
      "MIN",
      "SF",
  ...
  it("surfaces as the newest live package over the Sep 15 daily", () => {
    const tuesday = JSON.parse(
      readFileSync(
        path.join(
          __dirname,
          "../../../../content/writers/camp-desk-2026/2026-09-15.json",
        ),
        "utf8",
      ),
    ) as CampDeskDayFile;
    ...
    expect(shelf.live[0]?.desk_date).toBe("2026-09-16");
    expect(shelf.live.every((card) => card.desk_date === "2026-09-16")).toBe(
      true,
    );
    expect(shelf.activeDeskDate).toBe("2026-09-16");
    expect(shelf.archive.some((card) => card.desk_date === "2026-09-15")).toBe(
      true,
    );
    expect(shelf.deskStale).toBe(false);
    expect(displayClubDeskTitle(live.league_wrap.title)).toBe(
      "Club Desk — Wednesday, Sep 16",
    );
  });
});
...
describe("Sep 22 live Club Desk day", () => {
  const live = JSON.parse(
    readFileSync(
      path.join(
        __dirname,
  ...
  it("is a weekday daily package with six real-news clubs, singular preview_delta, and no X profile sources", () => {
    expect(live.desk_date).toBe("2026-09-22");
    expect(live.package).toBe("daily");
    expect(live.pinned).toBe(false);
    expect(live.source_type).toBe("kosedge-desk");
    expect(live.league_wrap.title).toBe("Club Desk — Tuesday, Sep 22");
    expect(live.league_wrap.storylines.length).toBeGreaterThanOrEqual(5);
    expect(live.league_wrap.storylines.length).toBeLessThanOrEqual(8);
    expect(live.team_notes.map((note) => note.team_id)).toEqual([
      "NYG",
    ...
    expect(
      live.preview_delta
        ?.filter((row) => row.team_id === "NYG" || row.team_id === "LAR")
        .every((row) => row.status === "touched"),
    ).toBe(true);
    expect(
      live.preview_delta
        ?.filter((row) => !["NYG", "LAR"].includes(row.team_id))
        .every((row) => row.status === "flagged"),
    ).toBe(true);
    expect(live.league_wrap.bottom_line.toLowerCase()).toContain("pass");
    expect(live.league_wrap.bottom_line).toContain("Over 2/5");
    expect(live.league_wrap.bottom_line).toContain("KEEP");
    expect(live.league_wrap.bottom_line).toContain("Coming soon ON");
    const blob = JSON.stringify(live);
    expect(blob).not.toMatch(/\bPLAY\b/);
    expect(blob).not.toMatch(/\bLEAN\b/);
    expect(blob).not.toMatch(/https?:\/\/(www\.)?(x|twitter)\.com/i);
  ...
  it("surfaces as the newest live package over the Sep 21 Monday", () => {
    const monday = JSON.parse(
      readFileSync(
        path.join(
          __dirname,
          "../../../../content/writers/camp-desk-2026/2026-09-21.json",
        ),
        "utf8",
      ),
    ) as CampDeskDayFile;
    ...
    expect(shelf.live[0]?.desk_date).toBe("2026-09-22");
    expect(shelf.live.every((card) => card.desk_date === "2026-09-22")).toBe(
      true,
    );
    expect(shelf.activeDeskDate).toBe("2026-09-22");
    expect(shelf.archive.some((card) => card.desk_date === "2026-09-21")).toBe(
      true,
    );
    expect(shelf.deskStale).toBe(false);
    expect(displayClubDeskTitle(live.league_wrap.title)).toBe(
      "Club Desk — Tuesday, Sep 22",
    );
  });
});

describe("Sep 23 live Club Desk day", () => {
  const live = JSON.parse(
    readFileSync(
      path.join(
        __dirname,
  ...
  it("is a weekday daily package with five real-news clubs, singular preview_delta, and no X profile sources", () => {
    expect(live.desk_date).toBe("2026-09-23");
    expect(live.package).toBe("daily");
    expect(live.pinned).toBe(false);
    expect(live.source_type).toBe("kosedge-desk");
    expect(live.league_wrap.title).toBe("Club Desk — Wednesday, Sep 23");
    expect(live.league_wrap.storylines.length).toBeGreaterThanOrEqual(5);
    expect(live.league_wrap.storylines.length).toBeLessThanOrEqual(8);
    expect(live.team_notes.map((note) => note.team_id)).toEqual([
      "NYG",
    ...
    expect(
      live.preview_delta
        ?.filter((row) => ["NYG", "CHI", "LAC"].includes(row.team_id))
        .every((row) => row.status === "touched"),
    ).toBe(true);
    expect(
      live.preview_delta
        ?.filter((row) => row.team_id === "ATL" || row.team_id === "ARI")
        .every((row) => row.status === "flagged"),
    ).toBe(true);
    expect(live.league_wrap.bottom_line.toLowerCase()).toContain("pass");
    expect(live.league_wrap.bottom_line).toContain("Over 2/5");
  ...
  it("surfaces as the newest live package over the Sep 22 daily", () => {
    const tuesday = JSON.parse(
      readFileSync(
        path.join(
          __dirname,
    ...
    expect(shelf.live[0]?.desk_date).toBe("2026-09-23");
    expect(shelf.live.every((card) => card.desk_date === "2026-09-23")).toBe(
      true,
    );
    expect(shelf.activeDeskDate).toBe("2026-09-23");
    expect(shelf.archive.some((card) => card.desk_date === "2026-09-22")).toBe(
      true,
    );
    expect(shelf.deskStale).toBe(false);
    expect(displayClubDeskTitle(live.league_wrap.title)).toBe(
      "Club Desk — Wednesday, Sep 23",
    );
  });
});
