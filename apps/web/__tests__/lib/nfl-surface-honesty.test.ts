...
describe("NFL awards vs futures source stamps", () => {
  it("exports distinct source names and as-of honesty copy", () => {
    ...
    expect(NFL_AWARDS_SOURCE_STAMP).toMatch(/not silently reconciled/i);
    ...
    expect(NFL_FUTURES_SOURCE_STAMP).toMatch(/not silently reconciled/i);
    expect(NFL_FUTURES_SOURCE_STAMP).toMatch(/odds not joined/i);
  ...
  it("awards and futures pages stamp their own source labels", () => {
    ...
    expect(awards).not.toMatch(/combined ranking|reconcile|mint.*odds/i);
    ...
    expect(futures).not.toMatch(/combined ranking|silently reconcile/i);
  });
});
...
describe("NFL depth charts are not claimed as live camp", () => {
  it("labels packaged model depth — not live Camp Desk", () => {
    ...
    expect(NFL_DEPTH_SOURCE_STAMP).toMatch(/do not invent a merge/i);
    expect(NFL_DEPTH_SOURCE_STAMP).not.toMatch(/live camp SoT|live depth SoT/i);
  });
