...
/** American → decimal odds. Decimal must be > 1. */
export function americanToDecimal(price: unknown): DecimalResult {
  const implied = americanToImpliedProb(price);
  if (implied.status !== "ok" || implied.impliedProb == null) {
    return {
      status: "DATA_GAP",
      decimal: null,
      american: null,
  ...
  const decimal = 1 / implied.impliedProb;
  if (!Number.isFinite(decimal) || decimal <= 1) {
    return {
      status: "DATA_GAP",
      decimal: null,
      american: isFiniteNumber(price) ? price : null,
      reason: "non_finite_decimal",
    };
  }
  return {
    status: "ok",
...
/** Decimal → American. Fail closed if decimal ≤ 1. */
export function decimalToAmerican(decimal: unknown): DecimalResult {
  if (!isFiniteNumber(decimal) || decimal <= 1) {
    return {
      status: "DATA_GAP",
      decimal: null,
      american: null,
      reason: "decimal_must_be_gt_1",
    };
  }
  const implied = 1 / decimal;
  const back = impliedProbToAmerican(implied);
  return {
    status: back.status,
    decimal,
    american: back.american,
    reason: back.reason,
  };
}
...
/**
 * Three-way no-vig (Regulation ML). Draw is a priced outcome.
 * Two-way helpers must not be used for this contract.
 */
export function removeVigThreeWay(
  homeAmerican: unknown,
  awayAmerican: unknown,
  drawAmerican: unknown,
): NoVigResult {
  const home = americanToImpliedProb(homeAmerican);
  const away = americanToImpliedProb(awayAmerican);
  const draw = americanToImpliedProb(drawAmerican);
  if (home.status !== "ok" || away.status !== "ok" || draw.status !== "ok") {
    return {
      status: "DATA_GAP",
      homeProb: null,
      awayProb: null,
  ...
  const total =
    (home.impliedProb as number) +
    (away.impliedProb as number) +
    (draw.impliedProb as number);
  if (!(total > 0) || !Number.isFinite(total)) {
    return {
      status: "DATA_GAP",
      homeProb: null,
      awayProb: null,
      drawProb: null,
      reason: "non_finite_three_way_total",
    };
  }
  return {
    status: "ok",
    homeProb: (home.impliedProb as number) / total,
    awayProb: (away.impliedProb as number) / total,
    drawProb: (draw.impliedProb as number) / total,
  };
}
...
/** Refuse mixing FG and F5 (or Game ML vs Regulation ML) in one calc. */
export function refuseCrossContractMix(
  a: MarketContractId,
  b: MarketContractId,
): { status: CalcStatus; reason?: string } {
