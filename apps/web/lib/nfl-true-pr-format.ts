...
export function qbPremiumChip(d: DriverLike): DriverChipView | null {
  if (!d) return null;
      ...
      : d.band
        ...
        : d.band_label && d.band_label !== "unavailable"
          ...
          : d.starter_name
            ? "Context only"
            : "—";
  const muted = !d.band && Boolean(d.starter_name || d.band_label === "Context only");
  return {
    key: "qb",
    title: "QB premium",
    value,
...
export function projectedSosChip(d: DriverLike): DriverChipView | null {
  ...
  return {
    ...
    framing:
      d.framing ||
      "Schedule outlook only — does not change intrinsic PR",
  };
}
...
export function driverChipsForTeam(drivers: {
  ...
  return [
    ...
    qbPremiumChip(drivers.qb_premium),
