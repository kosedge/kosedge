...
import {
  ...
  formatTotal,
  formatWinProb,
} from "@/lib/nba-fair-lines-format";
...
export type NbaFairLinesResponse = {
  gameDate: string;
  modelVersion: string;
...
export async function fetchNbaFairLines(params?: {
  gameDate?: string;
  modelVersion?: string;
  daysAhead?: number;
  source?: string;
}): Promise<NbaFairLinesResponse> {
  ...
  const url = new URL(`${base.replace(/\/+$/, "")}/nba/fair-lines`);
  ...
  // Chapter 4: prefer season-engine KEI on the Edge Board path.
