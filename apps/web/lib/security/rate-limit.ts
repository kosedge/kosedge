...
const apiLimiterMemory = new RateLimiterMemory({ points: POINTS.api, duration: 60 });
const authLimiterMemory = new RateLimiterMemory({ points: POINTS.auth, duration: 60 });
const strictLimiterMemory = new RateLimiterMemory({ points: POINTS.strict, duration: 60 });

// Redis-backed limiters (lazy-init when REDIS_URL is set); shared across instances
let redisLimiters: { api: Limiter; auth: Limiter; strict: Limiter } | null = null;
...
function getLimiterAndPoints(pathname: string): { limiter: Limiter; points: number } {
  ...
  if (pathname.startsWith("/api/edge-board")) {
    return { limiter: redis?.strict ?? strictLimiterMemory, points: POINTS.strict };
  }
...
export async function rateLimit(req: NextRequest): Promise<NextResponse | null> {
  ...
  try {
    ...
    const msBeforeNext = (rejRes as { msBeforeNext?: number })?.msBeforeNext ?? 60_000;
