...
export const NFL_CLV_POPULATION =
  "Population is +EV vs the open snapshot (moneyline: model win probability exceeds open implied probability on that side; total: |model − open| ≥ 1.0). Not PLAY-only. Not graded-only. Spread is not in this pipeline.";

export const NFL_CLV_TIMESTAMPS =
  "Open = first captured snapshot and close = last captured snapshot on that market — not tag/publish time vs a true closing book.";
