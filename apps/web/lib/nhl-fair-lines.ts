...
import {
  ...
  formatTotal,
  formatWinProb,
} from "@/lib/nhl-fair-lines-format";
...
export type NhlFairLinesResponse = {
  gameDate: string;
  modelVersion: string;
...
export async function fetchNhlFairLines(params?: {
  gameDate?: string;
  daysAhead?: number;
  source?: string;
}): Promise<NhlFairLinesResponse> {
  ...
  const url = new URL(`${base.replace(/\/+$/, "")}/nhl/fair-lines`);
  ...
  try {
    ...
    return {
      ...
      phase: payload.phase,
      preseason: Boolean(payload.preseason),
    };
  } catch (err) {
    return {
      gameDate: fallbackDate,
