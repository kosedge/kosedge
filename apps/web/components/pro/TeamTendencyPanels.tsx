...
export default function TeamTendencyPanels({
  ...
  const filteredSituational = situational.filter(
    (row) => row.situationType === activeSituation,
  );
    ...
    <div className="space-y-5">
      {usedFallback ? (
        <div className="rounded-xl border border-amber-400/30 bg-amber-400/10 p-3 text-sm text-amber-100">
          No real play-by-play exists yet for {requestedSeason} — showing the
          most recent completed season with real tendency data ({season})
          instead.
        </div>
      ) : null}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-kos-text/70">
              Real down/distance, score-state, and field-position splits from
              play-by-play — pass/run mix, pace, and efficiency in each
              situation. Not coverage-scheme labels (those don&apos;t exist in
              free data).
            </p>
          </div>
              ...
              <tbody>
                {filteredSituational.map((row) => (
                  <tr key={row.situationBucket} className="odd:bg-white/3">
                    ...
                    <td className="border-b border-white/5 px-3 py-2 text-sm text-kos-text/85">
                      {formatPercent(row.shotgunRate)}
                    </td>
                    <td className="border-b border-white/5 px-3 py-2 text-sm text-kos-text/85">
                      {formatPercent(row.noHuddleRate)}
                    </td>
                    <td className="border-b border-white/5 px-3 py-2 text-sm text-kos-text/85">
                      {formatSigned(row.epaPerPlay)}
                    </td>
                    <td className="border-b border-white/5 px-3 py-2 text-sm text-kos-text/85">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <article className="rounded-2xl border border-white/10 bg-black/30 p-4 sm:p-5">
          ...
          <p className="mt-1 text-sm text-kos-text/70">
            Real left/middle/right target-location mix for{" "}
            {perspective === "offense"
              ? "this team's offense"
              : "what this defense allows"}
            .
          </p>
          {direction ? (
        ...
        <article className="rounded-2xl border border-white/10 bg-black/30 p-4 sm:p-5">
          ...
          <p className="mt-1 text-sm text-kos-text/70">
            Real left/middle/right and end/guard/tackle gap mix on rush
            attempts.
          </p>
          {direction ? (
            <div className="mt-4 grid grid-cols-3 gap-2">
      ...
      <section className="rounded-2xl border border-white/10 bg-black/30 p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-kos-text/70">
              Real completion%, EPA/play, and CPOE splits for this team&apos;s
              quarterbacks under pressure vs. clean pocket, on early vs. money
              downs, and by score state.
            </p>
          </div>
                    ...
                    <tbody>
                      {rows.map((row) => (
                        ...
                        >
                          ...
                          <td className="border-b border-white/5 px-3 py-1.5 text-sm text-kos-gold">
                            {formatSigned(row.epaPerPlay)}
                          </td>
                          <td className="border-b border-white/5 px-3 py-1.5 text-sm text-kos-text/85">
...
function DirectionStat({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-lg border border-white/10 bg-white/5 px-2 py-2 text-center">
      <p className="text-[11px] uppercase tracking-wide text-kos-text/60">
        {label}
      </p>
