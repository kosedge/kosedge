...
import { formatIntelValueWithRank, formatTeamRecordWithRank } from "@/lib/nfl-intel";
...
function resolveRowIndex(row: NflIntelResponseRow, comparisonRows: NflIntelResponseRow[]): number {
...
export default function TeamIntelStatCards({
  ...
  const rankMaps = buildMetricRankMaps(
    rankingRows,
    CARD_FIELDS.map((field) => (field.key === "record" ? "win_pct" : field.key)),
  );
  ...
  return (
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {CARD_FIELDS.map((field) => (
        <article key={field.key} className="rounded-xl border border-white/10 bg-white/5 p-4">
          <p className="text-xs uppercase tracking-[0.14em] text-kos-text/60">{field.label}</p>
          <p className="mt-2 text-xl font-semibold text-kos-text">
            {row
              ? field.key === "record"
                ? formatTeamRecordWithRank(row, rankMaps.win_pct?.get(rowIndex))
                : formatIntelValueWithRank(row[field.key], rankMaps[field.key]?.get(rowIndex))
              : "—"}
          </p>
        </article>
      ))}
