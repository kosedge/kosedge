  ...
  return (
    <section className="space-y-2">
      {rows.slice(0, 12).map((row, index) => {
        const status =
          typeof row.report_status === "string" ? row.report_status : "Unknown";
        return (
          <article
            ...
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                <p className="text-sm font-semibold text-kos-text">
                  {formatIntelValue(row.player_name)}
                </p>
                <p className="text-xs text-kos-text/65">
                  {formatIntelValue(row.injury)} · Practice:{" "}
                  {formatIntelValue(row.practice_status)}
                </p>
              </div>
              <span
                className={`rounded-full border px-2 py-1 text-[11px] font-semibold ${impactTone(status)}`}
              >
                {status}
              </span>
            </div>
