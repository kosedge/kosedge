#!/usr/bin/env python3
"""NBA grading harness — freeze desk publications, grade after tip.

Infrastructure only. Does not rewrite KEI, projections, props packs, or tags.

Usage:
  python3 scripts/nba/grade_harness.py seed
  python3 scripts/nba/grade_harness.py summary
  python3 scripts/nba/grade_harness.py status
"""
...
STORE = ROOT / "data" / "nba_grades_2026.jsonl"
SCHEMA = ROOT / "docs" / "NBA_GRADE_SCHEMA.md"
SUMMARY_MD = ROOT / "data" / "ops" / "nba-grades-summary-2026.md"
SUMMARY_JSON = ROOT / "data" / "ops" / "nba-grades-summary-2026.json"
...
PROJ_PACK = (
    ...
    / "nba_season_engine"
    / "data"
    / "nba_player_projection_2026.json"
)
PROPS_MODULE = (
    ROOT
    / "services"
...
def build_schema_example_rows() -> List[Dict[str, Any]]:
    ...
    common = dict(
        season=SEASON,
        date="2026-10-20",
        game_id="schema-ex-bos-det-20261020",
    ...
    spread = _base_row(
        **common,
        player_id=None,
        market="spread",
        kei=-1.5,  # Ch4-shaped home-signed example (not a live emit)
        proj=None,
        open=-1.5,
        best_kick=-1.5,
        book="FanDuel",
    ...
    # Prop: Ch5 proj + Ch6 Best; tag stays n/a until a later tag PR.
    prop_pts = _base_row(
        **common,
    ...
    return [spread, total, prop_pts, prop_pra]
...
def cmd_seed(*, force: bool = False) -> int:
    ...
    rows = _read_rows()
    print("=== NBA grade harness seed ===")
    print(f"stamp:   {STAMP_FROZEN}")
    print(f"schema:  {SCHEMA}")
    print(f"store:   {STORE}")
    print(f"example rows: {n} (spread + total + pts + pra)")
...
def cmd_summary() -> int:
    ...
    lines = [
        ...
        summary["note"],
        "",
        "Ch4/Ch5/Ch6 packs unchanged. No publisher. No props PLAY.",
        "",
    ]
...
def main() -> int:
    ap = argparse.ArgumentParser(description="NBA grading harness")
