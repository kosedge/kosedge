#!/usr/bin/env python3
"""Build NBA Chapter 5 PlayerProjection pack from Ch2 minutes × rates × team pace.
...
Does not emit props tags, fantasy UI, or a new minute grid.
"""
...
GRID_PATH = DATA / "nba_minutes_grid_2026.json"
TALENT_PATH = DATA / "nba_player_talent_3y_2026.json"
REBASED_PATH = DATA / "nba_team_prior_rebased_2026_27.json"
OUT_PATH = DATA / "nba_player_projection_2026.json"
...
PER_GAME_URLS = {
    "2023-24": "https://www.basketball-reference.com/leagues/NBA_2024_per_game.html",
    "2024-25": "https://www.basketball-reference.com/leagues/NBA_2025_per_game.html",
    "2025-26": "https://www.basketball-reference.com/leagues/NBA_2026_per_game.html",
}
ADVANCED_URLS = {
...
UA = {"User-Agent": "Mozilla/5.0 (compatible; KosEdgeNbaCh5/1.0)"}
...
def _load_html(season: str, kind: str, url: str) -> str:
    ...
    path = Path(f"/tmp/nba_{year}_{kind}.html")
    if path.is_file() and path.stat().st_size > 100_000:
        return path.read_text(encoding="utf-8", errors="replace").replace("<!--", "").replace("-->", "")
    html = _get(url)
...
def _season_row(rows: list[dict]) -> dict:
    ...
    if len(real) == 1:
        return real[0]
    # Rare splits without combined marker — minute-weight per-game rates.
    ...
    for k in COUNT_KEYS:
        if k in out or any(k in r for r in real):
            out[k] = sum(float(r.get(k) or 0.0) * float(r.get("mp") or 0.0) for r in real) / mp
    if any("mp_per_g" in r for r in real):
        out["mp_per_g"] = sum(
            float(r.get("mp_per_g") or 0.0) * float(r.get("mp") or 0.0) for r in real
        ) / mp
    if any("usg" in r for r in real):
        out["usg"] = sum(float(r.get("usg") or 0.0) * float(r.get("mp") or 0.0) for r in real) / mp
    return out


def parse_per_game(html: str, season: str) -> list[dict]:
    table = re.search(r'<table[^>]*id="per_game_stats"[^>]*>.*?</table>', html, flags=re.S | re.I)
    if not table:
        raise SystemExit(f"missing per_game_stats for {season}")
    rows: list[dict] = []
    for tr in re.findall(r"<tr[^>]*>.*?</tr>", table.group(0), flags=re.S | re.I):
        ...
        try:
            g = int(float(fields.get("games") or 0))
        ...
        rows.append(
            {
                ...
                "team": team,
                "g": g,
                "mp": mp_g * g,  # season minutes proxy for weighting / combined pick
                "mp_per_g": mp_g,
                "pts": pts,
                "reb": reb,
                "ast": ast,
...
def parse_usg(html: str, season: str) -> list[dict]:
    ...
    if not table:
        raise SystemExit(f"missing advanced for {season}")
    rows: list[dict] = []
    for tr in re.findall(r"<tr[^>]*>.*?</tr>", table.group(0), flags=re.S | re.I):
        if 'data-stat="name_display"' not in tr:
            continue
        ...
        try:
            ...
            continue
        rows.append({"player_id": pid, "team": team, "mp": mp, "usg": usg, "season": season})
    return rows


def _weighted_mean(pairs: list[tuple[float, float]]) -> tuple[float, float]:
    """pairs of (value, weight) → (mean, weight_mass)."""
    num = den = 0.0
    for v, w in pairs:
        if w <= 0:
            continue
...
def _sigma_from_rates(rates: list[float], mean: float) -> float:
    ...
    if len(clean) >= 2:
        # population std across available seasons (weights already chose the mean)
        return float(pstdev(clean))
    if len(clean) == 1:
        # single-season: fractional uncertainty from the observed rate magnitude
...
def main() -> None:
    ...
    # player_id → season → per-min rates + usg
    ...
    for season, rows in per_by_season.items():
        ...
        for pid, plist in by_p.items():
            ...
            mp_g = float(row["mp_per_g"] or 0) or (float(row["mp"]) / max(int(row["g"]), 1))
            if mp_g <= 0:
                continue
            rates[pid][season] = {
    ...
    for season, rows in usg_by_season.items():
        ...
        for pid, plist in by_p.items():
            row = _season_row(plist)
            if pid not in rates or season not in rates[pid]:
                # usg-only seasons still usable for USG mean
                rates[pid].setdefault(season, {})
            rates[pid][season]["usg"] = float(row["usg"])

    league_pace = sum(float(t["pace"]) for t in rebased["teams"].values()) / 30.0
    ...
    for team in sorted(TEAM_CODES):
        ...
        for slot in slots:
            ...
            # weighted per-min means
            rate_means: dict[str, float] = {}
            rate_sigmas_pm: dict[str, float] = {}
            for key in ("pts_pm", "reb_pm", "ast_pm", "stl_pm", "blk_pm", "tov_pm", "fg3_pm"):
                pairs = []
                year_vals = []
                ...
                if mass <= 0:
                    # rookies / missing box: tiny positional prior from talent pack BPM only
                    mean = 0.05 if key == "pts_pm" else 0.02
                    year_vals = [mean]
                rate_means[key] = mean
            ...
            built.append(
                {
                    ...
                    "team": team,
                    "role": slot.get("role"),
                    "talent_bpm": float(slot.get("talent_bpm") or tal.get("talent_bpm") or 0),
                    "MIN": round(minutes, 4),
        ...
        # other counting stats: keep rate shape; mild pace already applied. Do not invent a second PTS path.
        team_pts = 0.0
        team_min = 0.0
        for p in built:
            raw = p.pop("_raw")
            ...
            # Scale companion counting σ with the same PTS identity scale for PTS;
            # leave other σ on their rate×min footing (still computed).
            ...
            ra = reb + ast
            sig = {
                "MIN": 0.0,  # opening-night grid is fixed in Ch2
            ...
            sig["PRA"] = round(math.sqrt(sig["PTS"] ** 2 + sig["REB"] ** 2 + sig["AST"] ** 2), 4)
        ...
        if drift > RESIDUAL_CAP + 1e-6:
            raise SystemExit(
                f"PTS identity failed for {team}: sum={team_pts:.4f} target={target_pts:.4f} "
                f"drift={drift:.4f} > cap={RESIDUAL_CAP}"
            )
    ...
    out = {
        ...
        "reads": [
            ...
            "BR per_game + advanced USG (3y)",
        ],
        "does_not": [
            "props / Edge PLAY/LEAN",
            "fantasy board",
            "new minute grid",
            "DARKO/EPM as the mean",
            "team if",
            "situation B2B",
            "CFB/NFL",
            "live props stub numbers",
        ],
