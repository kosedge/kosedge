...
SERVICE_CLEAN = ROOT / "services" / "model-service" / "data" / "cfb" / "warehouse" / "clean"
...
def _resolve_clean(prefer_hd: bool) -> Path:
    ...
    candidates.append(SERVICE_CLEAN)
...
def discover(prefer_hd: bool) -> Dict[str, Any]:
    ...
    if not games_path.is_file() or not closes_path.is_file():
        ...
        return {
            ...
            "candidates_checked": [
                ...
                str(MONOREPO_CLEAN),
                str(SERVICE_CLEAN),
                str(clean_dir(prefer_hd=prefer_hd)),
            ],
    ...
    for row in closes:
        ...
        close = _finite(row.get("close_spread_home"))
        if close is None:
            close = _finite(game.get("close_spread_home"))
        if close is None:
            n_missing_close += 1
            continue
        n_close += 1
    ...
    return {
        ...
        "close_source_note": "Uses finite close_spread_home only (NaN skipped).",
        "fit": False,
